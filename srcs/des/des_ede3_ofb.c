...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/08 00:32:25 by mguerrea          #+#    #+#             */
/*   Updated: 2020/09/08 13:54:20 by mguerrea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	des_encrypt_ede3_ofb(unsigned char buff[8], t_des *des, int len)
{
	uint64_t	block;
	uint64_t	k[2];
	int			i;
...
void	des_decrypt_ede3_ofb(unsigned char buff[8], t_des *des, int len)
{
	uint64_t	block;
	uint64_t	k[2];
	int			i;
...
int		ft_des_ede3_ofb(int argc, char **argv)
{
	...
	(void)argc;
	des_init(&des, des_decrypt_ede3_ofb);
	des.key_len = 192;
	if (des_parse(argv, &des))
		return (0);
	des.func = (des.mode) ? des_decrypt_ede3_ofb : des_encrypt_ede3_ofb;
	if (des.b64 && des.mode)
		des_read_b64(&des);
	else
		des_read(&des);
